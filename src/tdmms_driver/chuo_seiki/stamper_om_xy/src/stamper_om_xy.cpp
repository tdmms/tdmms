...
class stamper_om_xy_master {
 public:
  stamper_om_xy_master() {
    ros::NodeHandle node;
    ...
    strDelimiter = "\r\n";
    parameter_name = "/stamper_om_xy/ipaddr";
    ...
    parameter_name = "/stamper_om_xy/ipport";
    ...
    cmd_stp_move_ = node.subscribe(
        ...
        &stamper_om_xy_master::cmd_stp_move_Callback, this);
    cmd_jog_move_ = node.subscribe(
        "/stamper_om_xy_master/cmd_jog_move", 1,
        &stamper_om_xy_master::cmd_jog_move_Callback, this);
    cmd_abs_move_ = node.subscribe(
        "/stamper_om_xy_master/cmd_abs_move", 1,
        &stamper_om_xy_master::cmd_abs_move_Callback, this);
    cmd_stop_     = node.subscribe(
        "/stamper_om_xy_master/cmd_stop", 1,
        &stamper_om_xy_master::cmd_stop_Callback, this);
    cmd_init_     = node.subscribe(
        "/stamper_om_xy_master/cmd_init", 1,
        &stamper_om_xy_master::cmd_init_Callback, this);
    cmd_status_   = node.subscribe(
        "/stamper_om_xy_master/cmd_stat", 1,
        &stamper_om_xy_master::cmd_status_Callback, this);
    cmd_vel_      = node.subscribe(
        "/stamper_om_xy_master/cmd_vel", 1,
        &stamper_om_xy_master::cmd_vel_Callback, this);
    stat_pub_     = node.advertise<geometry_msgs::Point>(
        "/stamper_om_xy_master/status", 1);
    currpos_pub_  = node.advertise<geometry_msgs::Point>(
        "/stamper_om_xy_master/currpos_stream", 1);
    service       = node.advertiseService(
        "/stamper_om_xy_master/wait_for_stop",
        &stamper_om_xy_master::wait_for_stop, this);

    service_getcurpos = node.advertiseService(
        "/stamper_om_xy_master/get_currpos",
        &stamper_om_xy_master::get_currpos, this);
  }
  ...
  void cmd_vel_Callback(const stamper_om_xy::velocity vel) {
    std::ostringstream stream;
  ...
  bool get_currpos(stamper_om_xy::GetCurrentPos::Request &req,
                   stamper_om_xy::GetCurrentPos::Response &res) {
    int Send_Res;
    std::string response;
...
int main(int argc, char *argv[]) {
  ros::init(argc, argv, "stamper_om_xy_master");
  stamper_om_xy_master stamper_om_xy_master_;

  ros::Rate loop_rate(100);
  while(ros::ok()) {
    stamper_om_xy_master_.currpos_stream();
    ros::spinOnce();
    loop_rate.sleep();
  }
  return 0;
