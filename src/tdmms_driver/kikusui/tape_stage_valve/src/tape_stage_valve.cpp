...
class tape_stage_master {
 public:
  tape_stage_master() {
    ...
    strDelimiter = "\r\n";
    parameter_name = "/tape_stage_valve/ipaddr";
    ...
    parameter_name = "/tape_stage_valve/ipport";
    ...
    cmd_close_     = node.subscribe(
        "/tape_stage_valve/cmd_close", 1,
        &tape_stage_master::cmd_close_Callback, this);

    cmd_open_     = node.subscribe(
        "/tape_stage_valve/cmd_open", 1,
        &tape_stage_master::cmd_open_Callback, this);
  }
...
int main(int argc, char *argv[]) {
  ros::init(argc, argv, "arm_tape_stage");
  tape_stage_master tape_stage_master_;
  ros::spin();
  return 0;
}
