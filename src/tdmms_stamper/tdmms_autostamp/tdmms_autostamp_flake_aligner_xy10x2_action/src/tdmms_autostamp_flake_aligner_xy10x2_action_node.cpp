...
class autostamp_flake_aligner_xy10x2_class {
 protected:
  ros::NodeHandle node_;
  ros::Subscriber image_subscriber_main;
  actionlib::SimpleActionServer<
      tdmms_autostamp_flake_aligner_xy10x2_action::AlignFlakeXY10x2Action> as_;
  std::string action_name_;
  tdmms_autostamp_flake_aligner_xy10x2_action::AlignFlakeXY10x2Feedback feedback_;
  tdmms_autostamp_flake_aligner_xy10x2_action::AlignFlakeXY10x2Result result_;
 ...
 public:
  autostamp_flake_aligner_xy10x2_class(std::string name)
      : as_(node_, name,
            boost::bind(&autostamp_flake_aligner_xy10x2_class::executeAlignFlake,
                        this, _1),
            false),
        action_name_(name) {
  ...
  ~autostamp_flake_aligner_xy10x2_class(void) {}
  ...
  void executeAlignFlake(const tdmms_autostamp_flake_aligner_xy10x2_action::
                             AlignFlakeXY10x2GoalConstPtr &goalparam) {
    bool success = true;
    openWindows();
    image_subscriber_main = node_.subscribe(
        "/autostamp_camera_streamer/image_main", 100,
        &autostamp_flake_aligner_xy10x2_class::imageStreamCallback, this);
    ...
    try {
      ROS_INFO("URL:%s", goalparam->filename_target.data.c_str());
...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "tdmms_autostamp_flake_aligner_xy10x2_action");

  autostamp_flake_aligner_xy10x2_class flake_aligner(ros::this_node::getName());
