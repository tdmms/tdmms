...
class autostamp_flake_positioner_fast3_class {
 protected:
  ...
  tdmms_autostamp_flake_positioner_action_fast3::PositionFlakeFast3Feedback
      feedback_;
  tdmms_autostamp_flake_positioner_action_fast3::PositionFlakeFast3Result result_;
 ...
 public:
  explicit autostamp_flake_positioner_fast3_class(std::string name)
      : as_(node_, name,
            boost::bind(
                &autostamp_flake_positioner_fast3_class::executePositionFlake,
                this, _1),
            false),
        action_name_(name) {
  ...
  ~autostamp_flake_positioner_fast3_class(void) { db.close(); }
  ...
  void executePositionFlake(const tdmms_autostamp_flake_positioner_action_fast3::
                                PositionFlakeFast3GoalConstPtr &goalparam) {
    bool success = true;
    //    as_.acceptNewGoal();
    ...
    image_subscriber_main = node_.subscribe(
        ...
        &autostamp_flake_positioner_fast3_class::imageStreamCallback, this);
    ...
    q.prepare(
        ...
        "idimage = ?");
    q.addBindValue(goalparam->id_image.data);
    if (!q.exec()) {
    ...
    */
    double point_x_target = goalparam->point_target.x;
    double point_y_target = goalparam->point_target.y;
    //////////////////////////////////////////
    /// Extract Alignment Images
    /////////////////////////////////////////
    //q.prepare("CALL 2dmms_db.select_alignment_image(:idchip, :idsearch)");
    /*q.prepare("CALL 2dmms_db.select_alignment_image(?, ?)");
    ROS_ERROR("idchip:%d", goalparam->id_chip.data);
    ROS_ERROR("idsearch:%d", goalparam->id_search.data);
    //q.bindValue(":idchip", goalparam->id_chip.data);
    //q.bindValue(":idsearch", goalparam->id_search.data);
    q.addBindValue(goalparam->id_chip.data);
    q.addBindValue(goalparam->id_search.data);
    ...
    q.prepare("select * from 2dmms_db.temp");
    if (!q.exec()) {
      ROS_ERROR("QUERY ERROR, ERROR:%s SENT:%s",
                q.lastError().text().toStdString().c_str(),
                q.lastQuery().toStdString().c_str());
    ...
    while (q.next()) {
      point_x_alignment.push_back(static_cast<double>(q.value(0).toInt()));
      point_y_alignment.push_back(static_cast<double>(q.value(1).toInt()));
      filename_alignment << q.value(2).toString();
      ROS_INFO("%s", q.value(2).toString().toStdString().c_str());
    }
    */

    for(int i = 0 ; i < goalparam->pos_xs.data.size(); i ++) {
      ROS_INFO("%d,%d", goalparam->pos_xs.data[i], goalparam->pos_ys.data[i]);
      ROS_INFO("%f,%f", static_cast<double>(goalparam->pos_xs.data[i]), static_cast<double>(goalparam->pos_ys.data[i]));
      point_x_alignment.push_back(static_cast<double>(goalparam->pos_xs.data[i]));
      point_y_alignment.push_back(static_cast<double>(goalparam->pos_ys.data[i]));
    }

    QString aws_uris = QString::fromStdString(goalparam->aws_uris.data);
    filename_alignment = aws_uris.split(QLatin1Char(';'));
    for (int i = 0; i < filename_alignment.size(); ++i) {
      //printf("%s\n", filename_alignment.at(i).toStdString().c_str());
    }
    /*    std::stringstream ss = std::stringstream(goalparam->aws_uris.data);
    std::string buf;
    while (std::getline(ss, buf, ';')) {
      filename_alignment.push_back(buf);
    }
    */
    /////////////////////////////////////////////
    ////  Main Loop: Search
    ...
    for (int i = 0; i < filename_alignment.size(); ++i) {
      ...
      QUrl imageURL(filename_alignment.at(i));
      ...
      FindScaledShapeModel(ho_Image_Match, hv_ModelId, HTuple(0).TupleRad(),
                           HTuple(360).TupleRad(), 0.9, 1.2, 0.3, 1, 0.5,
      ...
      if (0 != ((hv_ModelScore.TupleLength()) != 0)) {
        ...
        ROS_INFO("%f,%f,%f,%f",point_x_target, point_x_alignment.at(i), point_y_target, point_y_alignment.at(i));
...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "tdmms_autostamp_flake_positioner_action_fast3");
  autostamp_flake_positioner_fast3_class flake_positioner(
